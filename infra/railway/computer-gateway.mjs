...
export function publicScreenUrl(raw, origin, secret, now = Date.now()) {
  ...
  if (local.protocol !== "http:" || local.hostname !== "127.0.0.1" ||
      Number(local.port) < 1024 || Number(local.port) > 65535 || local.pathname !== "/embed.html") {
...
export function screenTarget(raw, secret, now = Date.now()) {
  ...
  if (Number(port) < 1024 || Number(port) > 65535 || Number(expires) <= now ||
      Number(expires) > now + TTL || !equal(mac, signature(port, expires, secret))) return null;
...
export function createGateway({ secret, origin, supervisorPort = 7091 }) {
  ...
  const server = http.createServer((req, res) => {
    if (req.url === "/health" && req.method === "GET") {
      const probe = http.get({ hostname: "127.0.0.1", port: supervisorPort, path: "/health", timeout: 5000 }, upstream => {
    ...
    const control = /^\/computers(?:\/|\?|$)/.test(req.url ?? "") &&
      equal(req.headers.authorization, `Bearer ${secret}`);
    if (!screen && !control) { res.writeHead(401); res.end(); return; }
    if (screen && req.method !== "GET" && req.method !== "HEAD") { res.writeHead(405); res.end(); return; }
    ...
    const upstream = http.request({ hostname: "127.0.0.1", port: screen?.port ?? supervisorPort,
      path: screen?.path ?? req.url, method: req.method, headers, timeout: 330_000 }, response => {
      ...
      if (control && outHeaders.location && /\/screen(?:\?|$)/.test(req.url)) {
        try { outHeaders.location = publicScreenUrl(outHeaders.location, origin, secret); }
        catch { response.resume(); res.writeHead(502); res.end(); return; }
      }
      if (control && /\/screen-mode(?:\?|$)/.test(req.url) && response.statusCode === 200) {
        const chunks = []; let size = 0;
        response.on("data", chunk => {
          size += chunk.length;
          if (size > 65536) { response.destroy(); res.destroy(); return; }
          chunks.push(chunk);
        });
        response.on("end", () => {
          try {
            ...
            res.writeHead(200, outHeaders); res.end(body);
          } catch { res.writeHead(502); res.end(); }
        });
      } else { res.writeHead(response.statusCode ?? 502, outHeaders); response.pipe(res); }
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    const upstream = http.request({ hostname: "127.0.0.1", port: target.port,
      path: target.path, headers: headersFor(req.headers), timeout: 15_000 });
    upstream.on("upgrade", (response, remote, remoteHead) => {
      ...
      const timer = setTimeout(() => { remote.destroy(); socket.destroy(); }, target.expires - Date.now());
      timer.unref();
      socket.on("close", () => { clearTimeout(timer); remote.destroy(); });
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  createGateway({ secret: process.env.SANDBOX_SUPERVISOR_TOKEN,
    origin: process.env.COMPUTER_PUBLIC_ORIGIN }).listen(Number(process.env.PORT ?? 8080), "0.0.0.0");
}
