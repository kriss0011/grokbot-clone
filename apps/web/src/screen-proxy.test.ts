...
describe("noVNC proxy authorization", () => {
  ...
  it("proxies a remote Docker desktop and restores its sealed websocket token", () => {
    ...
    expect(resolveNovncTarget(view + "?view_only=false", "secret", 1_000)?.path).toBe(
      `${directory}embed.html?path=websockify%3Ftoken%3Dprivate&view_only=true`,
    );
    expect(
      resolveNovncTarget(view.replace("embed.html", "websockify?token=forged"), "secret", 1_000)
        ?.path,
    ).toBe(`${directory}websockify?token=private`);
    expect(
      resolveNovncTarget(view.replace("embed.html", "core/rfb.js"), "secret", 1_000)?.path,
    ).toBe(`${directory}core/rfb.js`);
    expect(
      resolveNovncTarget(view.replace(`${directory}embed.html`, "/computers"), "secret", 1_000)
        ?.path,
